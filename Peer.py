...
while choice != "J" and choice != "H":
    choice = input().upper()
    print("Invalid input. Please try again.") if choice != "J" and choice != "H" else None

#host peer logic
if(choice == "H"):
    #the choice to make the port number above 5000 is completely arbitrary, we can change this if any of you want
    print("Hosting a game now, enter a port you wish to listen on. Make sure it is higher than 5000")
    port = 5000
    while port <=5000:
        try:
            port = int(input())
    ...
    while accept != "Y":
        ...
        print("Enter 'Y' to proceed to battle with peer, enter anything else to ignore peer ")
        accept = input().upper()
